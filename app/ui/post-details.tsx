    ...
    <article className="max-w-2xl mx-auto px-6 py-12 flex flex-col gap-6">
      <header className="flex items-center gap-4">
        {user.img ? (
          ...
          <div className="w-12 h-12 rounded-full bg-zinc-100 flex items-center justify-center text-sm font-bold text-zinc-600 ring-2 ring-zinc-200">
            {initials}
          </div>
        )}
        <div className="flex flex-col leading-tight">
          <span className="font-semibold text-zinc-900">{user.name}</span>
          <span className="text-sm text-zinc-400">{publishedAt}</span>
        </div>
      </header>
      ...
      <Link
        href="./"
        className="self-start text-sm font-medium text-zinc-500 hover:text-zinc-900 flex items-center gap-1 transition-colors"
      >
        ← Back
      </Link>
    </article>
